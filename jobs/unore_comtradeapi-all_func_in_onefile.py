...
def unore_chk_need_updates(base_month:str, index_name:str):
    ...
    ### Save to csv
    mydf.to_csv(f"{csv_path}/{','.join(hscode.values())}_{base_month}_1availablity_all_{datetime.now().strftime('%Y%m%d')}.csv", index=False)
    df_reporter_code_latest.to_csv(f"{csv_path}/{','.join(hscode.values())}_{base_month}_2availablity_latest_{datetime.now().strftime('%Y%m%d')}.csv", index=False)
    ...
    esdf.to_csv(f"{csv_path}/{','.join(hscode.values())}_{base_month}_3es_latest_{datetime.now().strftime('%Y%m%d')}.csv")
...
def unore_update_to_es_with_spark(base_month:str, index_name:str, dict_reportercode_need_update:dict, update_all:bool=False):
    ...
    if return_df:
        df.to_csv(f"{csv_path}/{','.join(hscode.values())}_{base_month}_4actual_{datetime.now().strftime('%Y%m%d')}.csv", index=False)
        return spark_df
    else:
...
def del_old_csvfile(target_date_delete:str)->str:
    ...
    file_pattern = f"{csv_path}/{','.join(hscode.values())}*{date_to_delete.strftime('%Y%m%d')}.csv"
    print(file_pattern)
...
if __name__ == "__main__":
    ...
    print(f"list_month before (type/data): {type(args.list_month)}/{args.list_month}")
    list_month = ast.literal_eval(args.list_month)
    print(f"list_month after (type/data): {type(list_month)}/{list_month}")
    ...
    unore_all_main(list_month=list_month, index_name=index_name, update_all=args.update_all)
